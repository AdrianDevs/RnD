...
from surveys.questions.tests import create_brand, create_empty_survey, create_question, create_question_type, create_answer_choice, add_question_to_survey
...
class TestSurveyAnswer(APITestCase):
    ...
    def test_non_empty_list(self):
        ...
        response = self.client.post(self.uri, {'survey_id': survey.id, 'participant_id': participant.id,  # type: ignore
                                               'question_id': question.id, 'answer_choice_ids': [answer_choice_1.id]}, format="json") # type: ignore
...
class TestSurveyResponse(APITestCase):
    ...
    def test_create_survey_response(self):
        ...
        add_question_to_survey(survey.id, question_2.id, [answer_choice_2.id, answer_choice_3.id, answer_choice_4.id, answer_choice_5.id]) # type: ignore
        ...
        answers = [{'question_id': question_1.id, 'answer_choice_ids': [answer_choice_1.id]},  # type: ignore
                   {'question_id': question_2.id, 'answer_choice_ids': [answer_choice_3.id, answer_choice_4.id]}] # type: ignore

        data = {'survey_id': survey.id, 'first_name':participant_first_name, 'last_name':participant_last_name,  # type: ignore
                'location_id': participant_location.id, 'occupation_id': participant_occupation.id,  # type: ignore
                'age': participant_age, 'response_date': response_date, 'answers': answers}
    ...
    #     response = self.client.post(self.uri, {'survey_id': survey.id, 'participant_id': participant.id,  # type: ignore
    #                                            'response_date': '2021-01-01', 'answers': [{'question_id': question.id, 'answer_choice_id': answer_choice_1.id}]}) # type: ignore
    #     self.assertEqual(response.status_code, 201,
    #                      'Expected Response Code 201, received {0} instead.'
    #                      .format(response.status_code))
    ...
    #     self.assertEqual(len(result), 1)
    #     self.assertEqual(result[0]['survey']['name'], survey.name)
    #     self.assertEqual(result[0]['participant']['first_name'], participant.first_name)
    #     self.assertEqual(result[0]['answers'][