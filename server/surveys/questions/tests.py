...
from surveys.questions.serializers import AnswerChoiceSerializer, BrandSerializer, QuestionSerializerWriter, QuestionTypeSerializer, SurveyQuestionAndAnswerChoiceSerializerWriter, SurveySerializerWriter
...
def create_empty_survey(name: str, description: str, start_date: str, end_date: str, brand_id: int) -> Survey:
    survey = {'name': name, 'description': description, 'start_date': start_date, 'end_date': end_date, 'brand_id': brand_id}
...
def add_question_to_survey(survey_id: int, question_id: int, answer_choice_ids: list[int]) -> None:
    survey_question_and_answer_choice = {'survey_id': survey_id, 'question_id': question_id, 'answer_choice_ids': answer_choice_ids}
    survey_question_and_answer_choice_serializer = SurveyQuestionAndAnswerChoiceSerializerWriter(data=survey_question_and_answer_choice)
...
class TestSurveysList(APITestCase):
    ...
    def test_get(self):
      ...
      survey = {'name': 'Survey 1', 'description': 'Survey 1 Description', 'start_date': '2021-01-01', 'end_date': '2021-01-31', 'brand_id': brand.id} # type: ignore
      survey_serializer = SurveySerializerWriter(data=survey)
      if survey_serializer.is_valid():
        survey = survey_serializer.save()
      else:
